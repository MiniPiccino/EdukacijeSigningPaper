...
def generate_signature_document(event_id: str) -> tuple[str, bytes]:
    ...
    placeholder_context = {
        ...
        "Activity": event_activity,
        "__educationName, location, date __": f"{event_name}, {event_location}, {event_date}",
        "__educationName__": event_name,
        "__educationLocation__": event_location,
        "__educationDate__": event_date,
        "__EDUCATION_NAME__": event_name,
        "__EDUCATION_LOCATION__": event_location,
        "__EDUCATION_DATE__": event_date,
        "{{EDUCATION_NAME}}": event_name,
        "{{EDUCATION_LOCATION}}": event_location,
        "{{EDUCATION_DATE}}": event_date,
    }
    ...
    translation_map = {
        ...
        "E-mail adresa": "Email adresa / Email",
        "Signature": "Potpis / Signature",
        "SIGNATURE": "POTPIS / SIGNATURE",
        "POTPIS": "Potpis / Signature",
        "R.br.": "R.br. / No.",
        "Location": "Lokacija / Location",
        "LOCATION": "LOKACIJA / LOCATION",
        "Project activity": "Projektna aktivnost / Project activity",
        "PROJECT ACTIVITY": "PROJEKTNA AKTIVNOST / PROJECT ACTIVITY",
        "Napomena": "Napomena",
    }
    ...
    for ridx in range(data_row_start + len(signins), len(signature_table.rows)):
        row = signature_table.rows[ridx]
        cells = row.cells
        for key in ("name", "company", "phone", "email"):
            col_idx = column_map.get(key)
            if col_idx is not None and col_idx < len(cells):
                cells[col_idx].text = ""
        sig_idx = column_map.get("signature")
        if sig_idx is not None and sig_idx < len(cells):
            sig_cell = cells[sig_idx]
            sig_cell.text = ""
            _prepare_signature_cell(sig_cell)
